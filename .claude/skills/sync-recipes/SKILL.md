...
|---|---|---|
| X-Trans V | https://fujixweekly.com/fujifilm-x-trans-v-recipes/ | Supported |
| X-Trans IV | https://fujixweekly.com/fujifilm-x-trans-iv-recipes/ | Supported |
| X-Trans III | https://fujixweekly.com/fujifilm-x-trans-iii-recipes/ | Supported |
| X-Trans II | https://fujixweekly.com/fujifilm-x-trans-ii-recipes/ | Supported |
| X-Trans I | https://fujixweekly.com/fujifilm-x-trans-i-recipes/ | Supported |
...
Wait for the answer. All generations are supported — proceed directly to Step 2.

### Step 2 — Read current recipes from the generation file

Read the relevant `recipes-[gen].js` file (e.g. `recipes-iv.js` for X-Trans IV). Extract the `RECIPES_[GEN]` array (e.g. `RECIPES_IV`). Build an in-memory map keyed by `name` for fast lookup.

Also note the line number where the array ends (closing `]`) — you will need it to report patch insertion points.
...
3. The complete JS patch — formatted as comma-separated objects ready to append inside the `RECIPES_[GEN] = [...]` array in `recipes-[gen].js`

Then ask:
> Apply this patch to recipes-[gen].js automatically, or copy-paste manually?

If the user says apply: use Edit to insert the new objects at the end of the `RECIPES_[GEN]` array (before the closing `]`). Do not modify `index.html` or any other file.

If manual: output the patch in a code block and tell them exactly where to paste it (line number of the closing `]` of the `RECIPES_[GEN]` array in `recipes-[gen].js`).
...
- Never modify anything outside the `RECIPES_[GEN]` array when applying the patch — do not touch `index.html`
